...
const InitiatedInterestTable: React.FC<InitiatedListTableProps> = ({
  data,
}) => {
        ...
        <tbody>
          {currentItems?.length > 0 &&
            currentItems?.map((item, index) => {
              const { businessId, businessName, desc, date } = item;
              return (
                <tr className="border-b" key={businessId}>
...
export default InitiatedInterestTable;
